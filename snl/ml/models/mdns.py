...
class MDN:
    ...
    def __init__(self, n_inputs, n_hiddens, act_fun, n_outputs, n_components, rng=np.random):
        ...
        self.Wms = [theano.shared((rng.randn(self.net.n_outputs, n_outputs) / np.sqrt(self.net.n_outputs + 1)).astype(dtype), name='Wm'+str(i), borrow=True) for i in range(n_components)]
        self.bms = [theano.shared(rng.randn(n_outputs).astype(dtype), name='bm'+str(i), borrow=True) for i in range(n_components)]
        self.ms = [tt.dot(self.net.hs[-1], Wm) + bm for Wm, bm in zip(self.Wms, self.bms)]

        # mixture precisions
        # note that U here is an upper triangular matrix such that U'*U is the precision
        self.WUs = [theano.shared((rng.randn(self.net.n_outputs, n_outputs**2) / np.sqrt(self.net.n_outputs + 1)).astype(dtype), name='WU'+str(i), borrow=True) for i in range(n_components)]
        self.bUs = [theano.shared(rng.randn(n_outputs**2).astype(dtype), name='bU'+str(i), borrow=True) for i in range(n_components)]
        aUs = [tt.reshape(tt.dot(self.net.hs[-1], WU) + bU, [-1, n_outputs, n_outputs]) for WU, bU in zip(self.WUs, self.bUs)]
        ...
        # log probabilities
        self.y = tt.matrix('y')
        L_comps = [-0.5 * tt.sum(tt.sum((self.y-m).dimshuffle([0, 'x', 1]) * U, axis=2)**2, axis=1) + ldetU for m, U, ldetU in zip(self.ms, self.Us, ldetUs)]
    ...
    def initialize_mog(self, y, rng=np.random):
        ...
        for Wm, bm in zip(self.Wms, self.bms):
        ...
        for WU, bU in zip(self.WUs, self.bUs):
    ...
    def visualize_activations(self, x):
        ...
        for a, title in zip(activations, ['mixing coefficients', 'means', 'scale matrices']):
    ...
    def param_hist(self):
        ...
        for W, b, title in zip([all_Wa, all_Wm, all_WU], [all_ba, all_bm, all_bU], ['mixing coefficients', 'means', 'scale matrices']):
...
class MDN_SVI:
    ...
    def __init__(self, n_inputs, n_hiddens, act_fun, n_outputs, n_components, rng=np.random):
        ...
        # mixture means
        # the mean of each component is calculated separately. consider vectorizing this
        self.mWms = [theano.shared((rng.randn(self.net.n_outputs, n_outputs) / np.sqrt(self.net.n_outputs + 1)).astype(dtype), name='mWm'+str(i), borrow=True) for i in range(n_components)]
        self.mbms = [theano.shared(rng.randn(n_outputs).astype(dtype), name='mbm'+str(i), borrow=True) for i in range(n_components)]
        self.sWms = [theano.shared(-5.0 * np.ones([self.net.n_outputs, n_outputs], dtype=dtype), name='sWm'+str(i), borrow=True) for i in range(n_components)]
        self.sbms = [theano.shared(-5.0 * np.ones(n_outputs, dtype=dtype), name='sbm'+str(i), borrow=True) for i in range(n_components)]
        uams = [self.srng.normal((self.net.hs[-1].shape[0], n_outputs), dtype=dtype) for _ in range(n_components)]
        mams = [tt.dot(self.net.hs[-1], mWm) + mbm for mWm, mbm in zip(self.mWms, self.mbms)]
        sams = [tt.dot(self.net.hs[-1]**2, tt.exp(2*sWm)) + tt.exp(2*sbm) for sWm, sbm in zip(self.sWms, self.sbms)]
        zams = [tt.sqrt(sam) * uam + mam for sam, uam, mam in zip(sams, uams, mams)]
        self.ms = zams

        # mixture precisions
        # note that U here is an upper triangular matrix such that U'*U is the precision
        self.mWUs = [theano.shared((rng.randn(self.net.n_outputs, n_outputs**2) / np.sqrt(self.net.n_outputs + 1)).astype(dtype), name='mWU'+str(i), borrow=True) for i in range(n_components)]
        self.mbUs = [theano.shared(rng.randn(n_outputs**2).astype(dtype), name='mbU'+str(i), borrow=True) for i in range(n_components)]
        self.sWUs = [theano.shared(-5.0 * np.ones([self.net.n_outputs, n_outputs**2], dtype=dtype), name='sWU'+str(i), borrow=True) for i in range(n_components)]
        self.sbUs = [theano.shared(-5.0 * np.ones(n_outputs**2, dtype=dtype), name='sbU'+str(i), borrow=True) for i in range(n_components)]
        uaUs = [self.srng.normal((self.net.hs[-1].shape[0], n_outputs**2), dtype=dtype) for _ in range(n_components)]
        maUs = [tt.dot(self.net.hs[-1], mWU) + mbU for mWU, mbU in zip(self.mWUs, self.mbUs)]
        saUs = [tt.dot(self.net.hs[-1]**2, tt.exp(2*sWU)) + tt.exp(2*sbU) for sWU, sbU in zip(self.sWUs, self.sbUs)]
        zaUs = [tt.sqrt(saU) * uaU + maU for saU, uaU, maU in zip(saUs, uaUs, maUs)]
    ...
    def initialize_mog(self, y, rng=np.random):
        ...
        # initialize means approx with the data means
        for mWm, mbm in zip(self.mWms, self.mbms):
        ...
        diag_mask = np.eye(n_dim, dtype=bool)
        U[diag_mask] = np.log(U[diag_mask])
        for mWU, mbU in zip(self.mWUs, self.mbUs):
    ...
    def _create_constant_uas_across_datapoints(self):
        ...
        uams = [tt.tile(self.srng.normal((self.n_outputs,), dtype=dtype), [n_data, 1]) for _ in range(self.n_components)]
        uaUs = [tt.tile(self.srng.normal((self.n_outputs**2,), dtype=dtype), [n_data, 1]) for _ in range(self.n_components)]
        ...
        if rand:
            ...
            if self.eval_comps_f is None:
                self.eval_comps_f = theano.function(
                    inputs=[self.input],
                    outputs=[self.a] + self.ms + self.Us,
                    givens=list(zip(self.zas, self.mas))
                )

            comps = self.eval_comps_f(x)

        comps = [u[0] for u in comps] if one_datapoint else comps
        return comps[0], comps[1:self.n_components+1], comps[self.n_components+1:]
    ...
    def turn_into_conventional(self):
        ...
        for W, b, mW, mb in zip(net.net.Ws, net.net.bs, self.net.mWs, self.net.mbs):
        ...
        for Wm, bm, WU, bU, mWm, mbm, mWU, mbU in zip(net.Wms, net.bms, net.WUs, net.bUs, self.mWms, self.mbms, self.mWUs, self.mbUs):
...
def replicate_gaussian_mdn(net, n_rep, rng=np.random):
    ...
    if isinstance(net, MDN):
        ...
        for mog_p, p in zip(mog_net.net.parms, net.net.parms):
        ...
        for Wm, bm, WU, bU in zip(mog_net.Wms, mog_net.bms, mog_net.WUs, mog_net.bUs):
        ...
        for mWm, mbm, mWU, mbU, sWm, sbm, sWU, sbU in zip(mog_net.mWms, mog_net.mbms, mog_net.mWUs, mog_net.mbUs, mog_net.sWms, mog_net.sbms, mog_net.sWUs, mog_net.sbUs):
